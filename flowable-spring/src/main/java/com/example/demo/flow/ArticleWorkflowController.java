...
public class ArticleWorkflowController {
    ...
    protected static List<String> eventElementTypes = new ArrayList<>();
    protected static Map<String, InfoMapper> propertyMappers = new HashMap<>();
    ...
    @CrossOrigin()
    public JsonNode getModelJSON(@PathVariable String id) {

        ProcessInstance processInstance = runtimeService.createProcessInstanceQuery().processInstanceId(id).singleResult();
        if (processInstance == null) {
            throw new IllegalArgumentException("No process instance found with id " + id);
        }
        ...
        if (pojoModel == null || pojoModel.getLocationMap().isEmpty()) {
            throw new IllegalArgumentException("Process definition could not be found with id " + processInstance.getProcessDefinitionId());
        }
        ...
        if (!jobs.isEmpty()) {
            List<Execution> executions = runtimeService.createExecutionQuery().processInstanceId(id).list();
...
//        }
        ...
        ObjectNode displayNode = processProcessElements(pojoModel, completedElements, currentElements, breakpoints, id);
    ...
    protected ObjectNode processProcessElements(BpmnModel pojoModel, Set<String> completedElements, Set<String> currentElements, Collection<String> breakpoints, String processInstanceId) {
        ...
        if (!pojoModel.getPools().isEmpty()) {
            ...
            for (Pool pool : pojoModel.getPools()) {
                ...
                if (process != null && MyCollectionUtils.isNotEmpty(process.getLanes())) {
    ...
    protected void processElements(Collection<FlowElement> elementList, BpmnModel model, ArrayNode elementArray, ArrayNode flowArray,
        ...
        for (FlowElement element : elementList) {
            ...
            Collection<String> brokenExecutions = new ArrayList<>();// this.debuggerService.getBrokenExecutions(element.getId(), processInstanceId);
